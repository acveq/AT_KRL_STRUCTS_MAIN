...
void tree_t::split(tree_node_t* node, const int depth) {
        ...
        for (const int time_id : times) {
            ...
            for (const int prop_id : properties) {
                ...
                for (const int id : node_ids) {
                    ...
                    std::string::size_type sz = std::size(prop);   // alias of size_t
                    long long ll = std::stoll (prop,&sz,0);
                    qDebug()<<"works"<<ll;
                    current_objects.emplace_back(ll, id);
                }
        ...
        if (l_ptr && r_ptr) {
            ...
            string pr = dbw.get_property(best_obj_id, best_property_id, best_time_id);
            qDebug()<<QString::fromStdString(pr);

            std::string::size_type sz = 0;   // alias of size_t
            long long ll = std::stoll (pr,&sz,0);
            node->set_prop_val(ll);
