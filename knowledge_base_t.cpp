...
string temporal_object_t::to_string(const knowledge_base_t&) const {
	string res = "";
	res += "ИМЯ \"" + get_name() + "\"\n";
	res += "ГРУППА \"Объект\"\n";
	res += "АТРИБУТЫ\n";
...
string condition_attr_value_const_t::to_string(const knowledge_base_t& kf, bool in_brackets, const string&, const string&) const {
	string res = "";
	if (in_brackets)
		res += "(";
...
string condition_attr_value_attr_value_t::to_string(const knowledge_base_t& kf, bool in_brackets, const string&, const string&) const {
	string res = "";
	if (in_brackets)
		res += "(";
...
string complex_condition_t::to_string(const knowledge_base_t& kf, bool in_brackets, const string& left_delimeter, const string& right_delimeter) const {
	string res = "";
	if (in_brackets)
		res += "(";
...
string temporal_interval_t::to_string(const knowledge_base_t &kf) const {
	string res = "";
	res += "ИМЯ \"" + get_name() + "\"\n";
	res += "ГРУППА \"Интервал\"\n";
	res += "АТРИБУТЫ\n";
...
const temporal_entity_t* knowledge_base_t::add_object(const string& name) {
...
// const KBEntity* knowledge_field_t::_add_object(const string& name) {
//     if (_mp_objects.find(name) != _mp_objects.end())
//         return nullptr;

//     int cid = _objects.size();
//     _objects.push_back(new KBEntity(name));
//     mp_objects[objects.back()->get_name()] = objects.back();
//     return static_cast<KBEntity*>(_objects.back());
// }




const temporal_entity_t* knowledge_base_t::add_interval(const string& name) {
...
const temporal_entity_t* knowledge_base_t::add_rule(const string& name) {
...
const temporal_entity_t* knowledge_base_t::add_event(const string& name) {
...
void knowledge_base_t::add_attr(const temporal_entity_t* it, const string& attr_name, const string& attr_type) const {
...
void knowledge_base_t::set_start_condition(const temporal_entity_t* interval, condition_t* start) const {
...
void knowledge_base_t::set_end_condition(const temporal_entity_t* interval, condition_t* end) const {
	const temporal_interval_t* ptr = dynamic_cast<const temporal_interval_t*>(interval);
	if (ptr) {
		auto iter = mp_intervals.find(ptr->get_name());
		if (iter != mp_intervals.end()) {
...
void knowledge_base_t::set_duration_condition(const temporal_entity_t* interval, condition_t* duration) const {
	const temporal_interval_t* ptr = dynamic_cast<const temporal_interval_t*>(interval);
	if (ptr) {
		auto iter = mp_intervals.find(ptr->get_name());
		if (iter != mp_intervals.end()) {
...
string knowledge_base_t::get_full_attr_name_by_id(int obj_id, int attr_id) const {
	string res = "";
...
pair<string, string> knowledge_base_t::get_full_attr_name_by_id_as_pair(int obj_id, int attr_id) const {
	if (obj_id < (int)objects.size())
		return{ objects[obj_id]->get_name(), objects[obj_id]->get_attr_name_by_id(attr_id) };
	return{ "", "" };
}

int knowledge_base_t::get_object_id_by_name(const string& name) const {
	auto iter = mp_objects.find(name);
	if (iter != mp_objects.end())
...
int knowledge_base_t::get_interval_id_by_name(const string& name) const {
...
const temporal_object_t* knowledge_base_t::get_object(const string& name) const {
	auto iter = mp_objects.find(name);
	if (iter != mp_objects.end()) {
		return iter->second;
	}
...
const temporal_object_t* knowledge_base_t::get_object(const int id) const {
	if (id >= (int)objects.size())
		return nullptr;
	return objects[id];
}

const temporal_event_t* knowledge_base_t::get_event(const int id) const {
	if (id >= (int)events.size())
		return nullptr;
	return events[id];
}

const temporal_interval_t* knowledge_base_t::get_interval(const int id) const {
	if (id >= (int)intervals.size())
		return nullptr;
	return intervals[id];
}

const temporal_rule_t* knowledge_base_t::get_rule(const int id) const {
	if (id >= (int)rules.size())
		return nullptr;
	return rules[id];
}

string temporal_rule_t::to_string(const knowledge_base_t& kf) const {
	string res = "";
	res += "ИМЯ \"" + get_name() + "\"\n";
	res += "ТИП \"" + type + "\"\n";
	res += "ЕСЛИ\n";
...
string knowledge_base_t::get_interval_name_by_id(int id) const {
	if (id < (int)intervals.size()) {
...
int knowledge_base_t::get_event_id_by_name(const string& name) const {
...
string knowledge_base_t::get_event_name_by_id(int id) const {
	if (id < (int)events.size()) {
...
string condition_interval_duration_t::to_string(const knowledge_base_t& kf, bool in_brackets, const string&, const string&) const {
	string res = "";
	if (in_brackets)
		res += "(";
	res += "Интервал.";
...
string knowledge_base_t::to_string() const {
	string res = "";
	res += types_to_string();
...
string knowledge_base_t::types_to_string() const {
	string res = "";
...
string knowledge_base_t::objects_to_string() const {
	string res = "";
...
string knowledge_base_t::intervals_to_string() const {
	string res = "";
...
string knowledge_base_t::rules_to_string() const {
	string res = "";
...
string knowledge_base_t::events_to_string() const {
	string res = "";
...
void knowledge_base_t::add_condition(const temporal_entity_t* entity, condition_t* condition) const {
	{
...
void knowledge_base_t::add_action(const temporal_entity_t *rule, action_t *action) const {
...
string action_int_t::to_string(const knowledge_base_t& kf) const {
	string res = "";
	res += kf.get_full_attr_name_by_id(obj_id, attr_id);
	res += " " + op + " ";
...
string action_string_t::to_string(const knowledge_base_t &kf) const {
	string res = "";
	res += kf.get_full_attr_name_by_id(obj_id, attr_id);
	res += " " + op + " ";
	res += "\"" + value + "\"";
...
string condition_sum_equal_const_t::to_string(const knowledge_base_t& kf, bool in_brackets, const string&, const string&) const {
	string res = "";
	if (in_brackets)
		res += "(";
...
string true_condition_t::to_string(const knowledge_base_t&, bool in_brackets, const string&, const string&) const {
	return in_brackets ? "(1=1)" : "1=1";
}

string complex_action_t::to_string(const knowledge_base_t &kf) const {
	string res = "";
...
void complex_condition_t::load(std::istream& in) {
	...
	in >> cnt;
	for (int i = 0; i < cnt; ++i)
        data.push_back(knowledge_base_t::load_condition(in));
}
...
void temporal_interval_t::load(std::istream &in) {
	in >> id;
	in >> name;
    begin_condition = knowledge_base_t::load_condition(in);
    end_condition = knowledge_base_t::load_condition(in);
}
...
void complex_action_t::load(std::istream& in) {
	...
	in >> cnt;
	for (int i = 0; i < cnt; ++i)
        actions.push_back(knowledge_base_t::load_action(in));
}
...
void temporal_rule_t::load(std::istream &in) {
	in >> id;
	in >> name;
    condition = knowledge_base_t::load_condition(in);
    action = knowledge_base_t::load_action(in);
}

string knowledge_base_t::dump() const {
	string res = "";
...
void knowledge_base_t::dump(std::ostream& out) const {
	out << dump() << std::endl;
}

string knowledge_base_t::dump_objects() const {
	string res = "";
...
void knowledge_base_t::dump_objects(std::ostream& out) const {
	out << dump_objects() << std::endl;
}

string knowledge_base_t::dump_intervals() const {
	string res = "";
...
void knowledge_base_t::dump_intervals(std::ostream& out) const {
	out << dump_intervals() << std::endl;
}

string knowledge_base_t::dump_rules() const {
	string res = "";
...
void knowledge_base_t::dump_rules(std::ostream& out) const {
	out << dump_rules() << std::endl;
}

string knowledge_base_t::dump_events() const {
	string res = "";
...
void knowledge_base_t::dump_events(std::ostream& out) const {
	out << dump_events() << std::endl;
}

string knowledge_base_t::dump_types() const {
	string res = "types ";
...
void knowledge_base_t::dump_types(std::ostream& out) const {
	out << dump_types() << std::endl;
}

void knowledge_base_t::load_types(std::istream& in) {
	int cnt;
	in >> cnt;
	for (int i = 0; i < cnt; ++i) {
		string type_name;
...
condition_t* knowledge_base_t::load_condition(std::istream& in) {
	string type;
	in >> type;
...
action_t* knowledge_base_t::load_action(std::istream& in) {
	string type;
	in >> type;
...
void knowledge_base_t::load(std::istream& in) {
	string type;
...
void knowledge_base_t::load(const string& s) {
	std::istringstream in(s);
	load(in);
}

const vector<temporal_object_t*>& knowledge_base_t::getObjects() const
{
	return objects;
}
const vector<temporal_interval_t*>& knowledge_base_t::getIntervals() const
{
	return intervals;
}
const vector<temporal_rule_t*>& knowledge_base_t::getRules() const
{
	return rules;
}

string temporal_event_t::to_string(const knowledge_base_t& kf) const {
	string res = "";
	res += "ИМЯ \"" + get_name() + "\"\n";
	res += "ГРУППА \"Событие\"\n";
	res += "АТРИБУТЫ\n";
...
string event_interval_condition_t::to_string(const knowledge_base_t& kf, bool in_brackets, const string&, const string&) const {
	string res = "";
	if (in_brackets)
		res += "(";
...
string event_event_condition_t::to_string(const knowledge_base_t& kf, bool in_brackets, const string&, const string&) const {
	string res = "";
	if (in_brackets)
		res += "(";
...
string knowledge_base_t::to_xml() const {
	string res = "";
...
string knowledge_base_t::types_to_xml() const {
	string res = "";
	res += "<types>\n";
...
string knowledge_base_t::objects_to_xml() const {
	string res = "";
...
string knowledge_base_t::intervals_to_xml() const {
	string res = "";
	res += "<Intervals>\n";
...
string knowledge_base_t::events_to_xml() const {
	string res = "";
	res += "<Events>\n";
...
string knowledge_base_t::rules_to_xml() const {
	string res = "";
	res += "<rules>\n";
...
string temporal_object_t::to_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string temporal_interval_t::to_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string temporal_event_t::to_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string temporal_rule_t::to_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string condition_interval_duration_t::to_interval_xml(const knowledge_base_t&) const {
	return "ERROR! THIS MUST NOT BE CALLED! NO INTERVAL DURATION CONDITION IN INTERVAL START/END CONDITION!";
}

string condition_interval_duration_t::to_rule_xml(const knowledge_base_t&) const {
	return "ERROR! NOT IMPLEMENTED IN TEMPORAL REASONER! NO EXAMPLE OF XML!";
}

string condition_sum_equal_const_t::to_interval_xml(const knowledge_base_t&) const {
	return "ERROR! THIS MUST NOT BE CALLED! NO PLUS OPERATION IN INTERVAL START/END CONDITION!";
}

string condition_sum_equal_const_t::to_rule_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string condition_attr_value_const_t::to_interval_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string condition_attr_value_attr_value_t::to_interval_xml(const knowledge_base_t& kf) const {
	string res = "";
	res += "<EqOp Value=\"" + get_text_operation() + "\">\n";
...
string condition_attr_value_const_t::to_rule_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string condition_attr_value_attr_value_t::to_rule_xml(const knowledge_base_t& kf) const {
	string res = "";
	res += "<" + get_text_operation() + ">\n";
...
string event_interval_condition_t::to_interval_xml(const knowledge_base_t&) const {
	return "ERROR! THIS MUST NOT BE CALLED! NO TEMPORAL CONDITION IN IN INTERVAL START/END CONDITION!";
}

string event_interval_condition_t::to_rule_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string event_event_condition_t::to_interval_xml(const knowledge_base_t&) const {
	return "ERROR! THIS MUST NOT BE CALLED! NO TEMPORAL CONDITION IN IN INTERVAL START/END CONDITION!";
}

string event_event_condition_t::to_rule_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string complex_condition_t::to_interval_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string complex_condition_t::to_rule_xml(const knowledge_base_t& kf) const {
	string res = "";
	if (data.size() > 1)
		res += "<" + get_text_operation() + ">\n";
	for (condition_t* ptr : data)
...
string true_condition_t::to_interval_xml(const knowledge_base_t&) const {
	return "ERROR! THIS MUST NOT BE CALLED! NO TRUE CONDITION IN INTERVAL START/END CONDITION!";
}

string true_condition_t::to_rule_xml(const knowledge_base_t&) const {
	string res = "";
	res += "<eq>\n";
	res += "<value>1</value>\n";
	res += "<value>1</value>\n";
...
string complex_action_t::to_xml(const knowledge_base_t& kf) const {
	string res = "";
...
string action_int_t::to_xml(const knowledge_base_t& kf) const {
	string res = "";
	res += "<assign>\n";
	pair<string, string> obj_desc = kf.get_full_attr_name_by_id_as_pair(obj_id, attr_id);
	res += "<ref id=\"" + obj_desc.first + "\">\n";
...
string action_string_t::to_xml(const knowledge_base_t& kf) const {
	string res = "";
	res += "<assign>\n";
	pair<string, string> obj_desc = kf.get_full_attr_name_by_id_as_pair(obj_id, attr_id);
	res += "<ref id=\"" + obj_desc.first + "\">\n";
...
void knowledge_base_t::add_type(const string& type_name) {
...
void knowledge_base_t::add_type_value(const string& type_name, const string& value) {
	types[type_name].insert(value);
}

int knowledge_base_t::get_type_id(const string& type_name) const {
...
vector<string> knowledge_base_t::get_all_types() const {
	vector<string> res;
...
set<string> knowledge_base_t::get_type_values(const string& tname) const {
	set<string> res;
...
int knowledge_base_t::get_number_of_objects() const {
	return objects.size();
}

int knowledge_base_t::get_number_of_events() const {
	return events.size();
}

int knowledge_base_t::get_number_of_intervals() const {
	return intervals.size();
}

int knowledge_base_t::get_number_of_rules() const {
	return rules.size();
}
